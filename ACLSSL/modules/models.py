...
class ACL(nn.Module):
    def __init__(self, conf_file: str, device: str):
        ...
        # Init audio encoder
        checkpoint = torch.load(self.pretrain.audio_backbone, map_location=torch.device('cpu'))
    ...
    def encode_masked_vision(self, image: torch.Tensor, embedding: torch.Tensor):
